import {activePet} from './companions.mjs';
...
export const EVENTS={pack:{name:'Monster ambush',icon:'swords',description:'Two or three monsters share this battle. Defeat every foe to collect rewards.'},elite:{name:'Elite encounter',icon:'swords',description:'A stronger guardian challenges this run.'},boss:{name:'Final boss',icon:'crown',description:'Defeat this guardian to win the chapter.'},battle:{name:'Monster encounter',icon:'swords',description:'A creature blocks the path. Win a battle to earn gold and a reward.'},camp:{name:'A moment to reflect',icon:'flame',description:'Recover 35 health and one focus charge. Read a passage by the campfire before returning to the board.'},blessing:{name:'A light on the path',icon:'sparkles',description:'Discover a shrine and choose a skill for this run.'},market:{name:'The traveler’s market',icon:'store',description:'Trade the gold you earned for supplies for your journey.'},storm:{name:'Through the storm',icon:'cloud-lightning',description:'The winds sweep across your path. A ward protects your gold; otherwise you lose up to 10 gold.'},treasure:{name:'A hidden treasure',icon:'gem',description:'Roll a free treasure die to earn bonus run tokens.'}};
...
export function combatTurn(state,action,rng=Math.random){
 ...
 const pet=state.phase==='run'?activePet(state):null;
 if(pet&&b.turn%3===0){const effect={id:pet.id,name:pet.name,heal:0,damage:0,shield:0};if(pet.id==='dove'){effect.heal=Math.min(maxHealth(state)-next.health,4+pet.level*2);next.health+=effect.heal;}if(pet.id==='eagle'){effect.damage=4+pet.level*3;damage+=effect.damage;}if(pet.id==='lion'){effect.shield=5+pet.level*3;b.shield=(b.shield||0)+effect.shield;b.lastTurn.shieldGained+=effect.shield;}b.lastTurn.companion=effect;b.effects.push(pet.name+' assists');}
 ...
 if(b.enemies){
  let target=b.enemies[b.target]?.hp>0?b.target:b.enemies.findIndex(e=>e.hp>0);b.target=target;const main=b.enemies[target],splash=action==='radiance'?.5:skillLevel(state,'cleave')*.2;
  b.lastTurn.damageDealt=Math.min(main.hp,damage);main.hp=Math.max(0,main.hp-damage);
  if(splash){let splashTotal=0;for(const [i,e] of b.enemies.entries())if(i!==target&&e.hp>0){const hit=Math.min(e.hp,Math.ceil(damage*splash));e.hp-=hit;splashTotal+=hit;}b.lastTurn.damageDealt+=splashTotal;if(splashTotal)b.effects.push(`Cleave hits the group +${splashTotal}`);}
  b.hp=b.enemies.reduce((sum,e)=>sum+e.hp,0);
 }else{b.lastTurn.damageDealt=Math.min(b.hp,damage);b.hp=Math.max(0,b.hp-damage);}b.lastAction=action;
 ...
 const rawAttack=Math.round((8+state.stage*4+(b.intent==='heavy'?10:0)+(b.variant===2&&b.hp<b.max/2?4:0)+(state.phase==='run'?Math.floor(state.run.turn*.4)+(b.kind==='boss'?9:b.kind==='elite'?4:0):0))*(1+career(state).runs*.3)*(b.enemies?b.enemies.filter(e=>e.hp>0).length*.65:1));const attack=Math.max(Math.ceil(rawAttack*.25),rawAttack-skillLevel(state,'frost')*2-gear.defense);
...
export const SKILLS=[
 {id:'cleave',name:'Sweeping Light',icon:'swords',rarity:'common',max:3,desc:l=>`In group battles, attacks also deal ${l*20}% damage to every other monster. Ultimates always splash 50%.`},
